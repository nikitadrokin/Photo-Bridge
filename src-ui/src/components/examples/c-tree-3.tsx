...
import { IconFolderOpen, IconFolder, IconFile } from '@tabler/icons-react';
            ...
            <TreeItem key={item.getId()} item={item}>
              <TreeItemLabel className="before:bg-background relative before:absolute before:inset-x-0 before:-inset-y-0.5 before:-z-10">
                <span className="flex items-center gap-2">
                  {item.isFolder() ? (
                    item.isExpanded() ? (
                      <IconFolderOpen className="text-muted-foreground pointer-events-none size-4" />
                    ) : (
                      <IconFolder className="text-muted-foreground pointer-events-none size-4" />
                    )
                  ) : (
                    <IconFile className="text-muted-foreground pointer-events-none size-4" />
                  )}
                  {item.getItemName()}
                </span>
              </TreeItemLabel>
